...
if (selection == 'About the Dataset'):
 ...
 'chronic Lung Disease'}'''
    ...
    with tab5:
        ...
        st.subheader("Plotting the Traning vs Test (Accuracy and Loss)")
        col1, col2 = st.columns(2)

        with col1:
            acc = Image.open("images/acc.png")

            st.image(acc, caption='Traning vs Test Accuracy',width=350)

        with col2:
            loss = Image.open("images/loss.png")

            st.image(loss, caption='Traning vs Test Loss',width=350)

        st.write("As we can see from the above diagram that our Models performs well on the Training as well as Test Data")
...
if (selection == 'Lung Cancer Prediction'):
    ...
    idn = st.slider('Select any patient from Testing Data', 0, 200, 25)
    a=concate_data.iloc[idn]
    st.write("Displaying vales of patient ", idn)
    ...
    if st.button('Lung Cancer Test Result'):
        cancer_prediction = cancer_model.predict([[Age, Gender, AirPollution, Alcoholuse, BalancedDiet, Obesity, Smoking, PassiveSmoker, Fatigue, WeightLoss,ShortnessofBreath, Wheezing, SwallowingDifficulty,ClubbingofFingerNails, FrequentCold, DryCough, Snoring]])                          
        
        if (cancer_prediction[0] == 'High'):
          cancer_diagnosis = 'The person has lung cancer'
          st.error(cancer_diagnosis)

        elif(cancer_prediction[0] == 'Medium'):
          cancer_diagnosis = 'The person has a chance of having lung cancer'
          st.warning(cancer_diagnosis)
        else:
          cancer_diagnosis = 'The person does not have lung cancer'
          st.balloons()
...
if selection == 'CNN Based disease Detection':
    def loading_model():
        model_loader = load_model("C:\\Users\\user\\Desktop\\Lung_Cancer_Streamlit\\lungcancer.h5", compile=False)
        model_loader.compile(optimizer="adam", loss="categorical_crossentropy")
        return model_loader
    ...
    if temp is not None:
        ...
        try:
            ...
            if file_type not in ['jpeg', 'png', 'jpg']:
                st.error("Invalid file type! Please upload a proper image file (JPEG, PNG).")
                st.stop()
            ...
            # Load image using OpenCV for further validation
            img_cv2 = cv2.imdecode(np.frombuffer(buffer.getvalue(), np.uint8), cv2.IMREAD_GRAYSCALE)

            # Check if the image is grayscale
            if img_cv2 is None or len(img_cv2.shape) != 2:
                st.error("Invalid image type! Please upload a grayscale CT-Scan image.")
                st.stop()

            # Validate the image dimensions
            if img_cv2.shape[0] < 100 or img_cv2.shape[1] < 100:
                st.error("Image resolution is too low. Please upload a higher-quality CT-Scan image.")
                st.stop()

            # Check for brightness levels to avoid random bright or dark images
            if np.mean(img_cv2) < 10 or np.mean(img_cv2) > 240:
                st.error("The image appears too dark or too bright to be a valid CT-Scan.")
                st.stop()

            # Edge detection to ensure the image has medical details
            edges = cv2.Canny(img_cv2, 50, 150)
            edge_density = np.mean(edges)
            if edge_density < 10:
                st.error("The image lacks sufficient medical details. Please upload a valid CT-Scan image.")
                st.stop()

            # Entropy check to measure image complexity (medical images should not be too simple)
            def calculate_entropy(img_gray):
                # Computes the entropy of an image to measure complexity
                hist, _ = np.histogram(img_gray, bins=256, range=(0, 256), density=True)
                hist = hist[hist > 0]
                entropy = -np.sum(hist * np.log2(hist))
                return entropy
            
            entropy = calculate_entropy(img_cv2)
            
            
            if entropy < 4.0:
                st.error("The image appears too simple and may not be a valid medical image. Please upload a valid CT-Scan image.")
                st.stop()

        except (UnidentifiedImageError, OSError):
            st.error("Invalid image! Please upload a proper CT-Scan image.")
            temp_file.close()
            st.stop()

        # Preprocess the image for prediction
        ved_img = image.load_img(temp_file.name, target_size=(224, 224), color_mode='rgb')
        pp_ved_img = image.img_to_array(ved_img) / 255.0
        ...
        # Adding a confidence threshold to handle random images
        confidence = hardik_preds[0][0]
        if confidence >= 0.5 and confidence < 0.8:
            st.warning("The model is not confident in its prediction. This might not be a valid CT-scan.")
        elif confidence >= 0.8:
            out = ('I am {:.2%} sure that this is a Normal Case'.format(confidence))
            st.balloons()
            st.success(out)
        else:
            out = ('I am {:.2%} sure that this is a Lung Cancer Case'.format(1 - confidence))
            st.error(out)

    else:
        st.text("Please upload a CT-Scan image.")
